...
@app.get("/código/&{codigo}")
def read_item(codigo: str):  
    while True:
        try:
            sql = "SELECT Código ,Nombre, Creado, Usuario_creador, Actualizado, Actualizar_usuario FROM clients_export__2_ WHERE Código = '%s'" % codigo.upper()
            df = pd.read_sql_query(sql, con=conexion)
            return { 'State'              :  0 ,
                     'Descripción'       : 'Ruta encontrada, consulta ejecutada exitosamente.',
            ...
            return {'State': 1 , 'Descripción': 'Ruta no encontrada, intente nuevamente con otra.'}